        ...
        html! {
            <div id="cookieConsentBanner" class="fixed bottom-0 left-0 right-0 bg-gray-800 text-white p-4 z-50">
                <div class="grid sm:grid-cols-1 md:grid-cols-12 gap-2 items-center justify-between">
                    ...
                    <Link<Route> classes={"transition hover:text-gray-400 underline"} to={Route::PrivacyPolicy}>{"Learn more"}</Link<Route>>
                    // { " or " }
                    // <a href={settings_link} class="underline text-blue-300 hover:text-blue-500">{ "manage your settings" }</a>
                    { "." }</p>
